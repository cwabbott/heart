class Metric < ActiveRecord::Base
  ...
  def self.find_or_create(date,average)
    Metric.find_or_create_by_fulldate_and_movingaverage(date,average)
  end
  ...
  def self.aggregate_daily_moving_averages(days=30, where="WHERE fulldate > SUBDATE(NOW(), 90)", groupby=nil, measurements=true)
    if groupby.nil? 
    ...
    self.find_by_sql("SELECT fulldate, fulldate AS d, year, monthofyear AS month, dayofyear AS day, '#{days}' AS movingaverage #{m_with_a} FROM metrics #{where} AND movingaverage = #{days} #{groupby} ORDER BY d ASC")
  end

  def self.aggregate_daily_sums(days=30, where="WHERE fulldate > SUBDATE(NOW(), 90)", groupby=nil, measurements=true)
  ...
  def moving_averages!(days)
    ...
    metric_with_calculated_averages = Metric.find_by_sql("SELECT fulldate, fulldate AS d, year, monthofyear AS month, dayofyear AS day, '#{days}' AS movingaverage, #{subselects} FROM metrics WHERE fulldate = '#{fulldate}' AND movingaverage = 0")
