  ...
  */
  ...
  return {
    ...
    description : function(bool_remove){
      ...
      if(!remove){
        $.ajax({
          type : "POST",
          cache : false,
          url  : HEARTENGINE.annotateDescriptionPath(),
          dataType: 'script',
    ...
    link : function(){
      ...
      $('#linkto').val(HEARTENGINE.baseUrl()+HEARTENGINE.defaultPath()+"?review=1&" + $('#heartform input:checkbox').serialize().toString() + date_from + date_to + description + yearly_stacked);
    ...
    image : function(){
      ...
      result = $.ajax({
        ...
        success : function(res){
          ...
          window.open(HEARTENGINE.imagesPath() + '/' + image.id + '.png');
        },
        headers : {
          'X-Transaction': 'POST Example',
          'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content')
